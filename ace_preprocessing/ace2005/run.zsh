...
# Manual Fix CNN_IP_20030408.1600.04
python3 manual_fix.py
# adjust offsets
...
mkdir corpus/train corpus/dev corpus/test corpus/train+dev
for i in `cat split/train`
do
cp corpus/$i.split.ann corpus/train
cp corpus/$i.split.txt corpus/train
cp corpus/$i.ann corpus/train
cp corpus/$i.txt corpus/train
done
for i in `cat split/dev`
do
cp corpus/$i.split.ann corpus/dev
cp corpus/$i.split.txt corpus/dev
cp corpus/$i.ann corpus/dev
cp corpus/$i.txt corpus/dev
done
for i in `cat split/test`
do
cp corpus/$i.split.ann corpus/test
cp corpus/$i.split.txt corpus/test
cp corpus/$i.ann corpus/test
cp corpus/$i.txt corpus/test
done
cp corpus/train/* corpus/dev/* corpus/train+dev/
