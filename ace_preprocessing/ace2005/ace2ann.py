...
doc = open(sys.argv[2], encoding="utf8").read()
doc = re.sub(r"<[^>]+>", "", doc)
doc = re.sub(r"(\S+)\n(\S[^:])", r"\1 \2", doc)
...
for ne in named_entities.values():
    assert doc[int(ne[2]):int(ne[3])].replace("&AMP;", "&").replace("&amp;", "&").replace(" ", "") == ne[4].replace(" ",
                                                                                                                    ""), "%s <=> %s" % (
    doc[int(ne[2]):int(ne[3])], ne[4])

out = open(sys.argv[3], 'w', encoding="utf8")
out.write(doc)
out.close()
